...
describe("MultiSend", async () => {
    ...
    describe("multiSend", async () => {
        it("should enforce delegatecall to MultiSend", async () => {
            ...
            const killLib = await (0, setup_1.deployContract)(user1, source);
            const nestedTransactionData = (0, multisend_1.encodeMultiSend)([(0, execution_1.buildContractCall)(killLib, "killme", [], 0)]);
            const multiSendCode = await hardhat_1.default.ethers.provider.getCode(multiSend.address);
            await (0, chai_1.expect)(multiSend.multiSend(nestedTransactionData)).to.be.revertedWith("MultiSend should only be called via delegatecall");
            (0, chai_1.expect)(await hardhat_1.default.ethers.provider.getCode(multiSend.address)).to.be.eq(multiSendCode);
        });
        ...
        it("can execute contract delegatecalls", async () => {
            const { safe, multiSend, storageSetter } = await setupTests();
            const txs = [(0, execution_1.buildContractCall)(storageSetter, "setStorage", ["0xbaddad"], 0, true)];
            const safeTx = (0, multisend_1.buildMultiSendSafeTx)(multiSend, txs, await safe.nonce());
            await (0, chai_1.expect)((0, execution_1.executeTx)(safe, safeTx, [await (0, execution_1.safeApproveHash)(user1, safe, safeTx, true)])).to.emit(safe, "ExecutionSuccess");
