...
describe("Upgrade from Safe 1.1.1", () => {
    ...
    const setupTests = hardhat_1.deployments.createFixture(async ({ deployments }) => {
        ...
        const singleton111 = (await (await user1.sendTransaction({ data: safeDeployment_json_1.default.safe111 })).wait()).contractAddress;
        const singleton140 = (await (0, setup_1.getSafeSingleton)()).address;
        const factory = await (0, setup_1.getFactory)();
        const saltNonce = 42;
        const proxyAddress = await (0, proxies_1.calculateProxyAddress)(factory, singleton111, "0x", saltNonce);
        ...
        (0, chai_1.expect)(await safe.VERSION()).to.be.eq("1.1.1");
        ...
        (0, chai_1.expect)(await safe.VERSION()).to.be.eq("1.4.1");
        return {
            migratedSafe: safe,
            mock,
