"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
require("hardhat-deploy");
require("@nomiclabs/hardhat-ethers");
var config_1 = require("hardhat/config");
var solc_1 = require("../utils/solc");
(0, config_1.task)("codesize", "Displays the codesize of the contracts")
    .addParam("skipcompile", "should not compile before printing size", false, config_1.types.boolean, true)
    .addParam("contractname", "name of the contract", undefined, config_1.types.string, true)
    .setAction(function (taskArgs, hre) { return __awaiter(void 0, void 0, void 0, function () {
    var contracts, _i, contracts_1, contract, artifact;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                if (!!taskArgs.skipcompile) return [3 /*break*/, 2];
                return [4 /*yield*/, hre.run("compile")];
            case 1:
                _a.sent();
                _a.label = 2;
            case 2: return [4 /*yield*/, hre.artifacts.getAllFullyQualifiedNames()];
            case 3:
                contracts = _a.sent();
                _i = 0, contracts_1 = contracts;
                _a.label = 4;
            case 4:
                if (!(_i < contracts_1.length)) return [3 /*break*/, 7];
                contract = contracts_1[_i];
                return [4 /*yield*/, hre.artifacts.readArtifact(contract)];
            case 5:
                artifact = _a.sent();
                if (taskArgs.contractname && taskArgs.contractname !== artifact.contractName)
                    return [3 /*break*/, 6];
                console.log(artifact.contractName, Math.max(0, (artifact.deployedBytecode.length - 2) / 2), "bytes (limit is 24576)");
                _a.label = 6;
            case 6:
                _i++;
                return [3 /*break*/, 4];
            case 7: return [2 /*return*/];
        }
    });
}); });
(0, config_1.task)("yulcode", "Outputs yul code for contracts")
    .addParam("contractname", "name of the contract", undefined, config_1.types.string, true)
    .setAction(function (taskArgs, hre) { return __awaiter(void 0, void 0, void 0, function () {
    var contracts, _i, contracts_2, contract, buildInfo, solcjs, compiled, output;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, hre.artifacts.getAllFullyQualifiedNames()];
            case 1:
                contracts = _a.sent();
                _i = 0, contracts_2 = contracts;
                _a.label = 2;
            case 2:
                if (!(_i < contracts_2.length)) return [3 /*break*/, 6];
                contract = contracts_2[_i];
                if (taskArgs.contractname && !contract.endsWith(taskArgs.contractname))
                    return [3 /*break*/, 5];
                return [4 /*yield*/, hre.artifacts.getBuildInfo(contract)];
            case 3:
                buildInfo = _a.sent();
                if (!buildInfo)
                    return [2 /*return*/];
                console.log({ buildInfo: buildInfo });
                buildInfo.input.settings.outputSelection["*"]["*"].push("ir", "evm.assembly");
                return [4 /*yield*/, (0, solc_1.loadSolc)(buildInfo.solcLongVersion)];
            case 4:
                solcjs = _a.sent();
                compiled = solcjs.compile(JSON.stringify(buildInfo.input));
                output = JSON.parse(compiled);
                console.log(output.contracts[contract.split(":")[0]]);
                console.log(output.errors);
                _a.label = 5;
            case 5:
                _i++;
                return [3 /*break*/, 2];
            case 6: return [2 /*return*/];
        }
    });
}); });
