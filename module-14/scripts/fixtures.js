...
const chains_1 = require("../dApp/src/lib/config/chains");
...
const client = (0, viem_1.createWalletClient)({
    account,
    chain: chains_1.virtual_mainnet,
    transport: (0, viem_1.http)(chains_1.virtual_mainnet.rpcUrls.default.http[0]),
});
...
async function placeOrders() {
    ...
    for (let i = 0; i < 2; i++) {
        ...
        const amount = pair.tokenIn === USDC || pair.tokenIn === DAI
            ? (0, viem_1.parseUnits)(((i + 1) * 100).toString(), 6) // Stablecoins have 6 decimals
            : (0, viem_1.parseEther)(((i + 1) * 0.01).toString()); // ETH/BTC tokens have 18 decimals
        // Dynamic target prices
        const basePrice = orderType === 0 ? 1800 : 2200;
        const targetPrice = (0, viem_1.parseEther)((basePrice + (i * 50)).toString());
        // Varied expiry times
